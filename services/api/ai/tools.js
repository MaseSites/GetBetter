...
const TIME = { type: 'string', pattern: '^([01]\\d|2[0-3]):[0-5]\\d$', description: 'Uhrzeit HH:MM (24 h)' };
...
const TOOLS = [
  {
    ...
    parameters: object(
      { title: TITLE, date: DATE, start: TIME, end: TIME, location: { type: 'string', maxLength: 120 }, note: NOTE },
      ['title', 'date'],
    ),
  },
  {
    name: 'move_event',
    apps: CALENDAR_APPS,
    description: 'Verschiebt einen Termin aus der Liste auf einen anderen Tag oder eine andere Zeit.',
  ...
  },
  {
    name: 'create_task',
    apps: ['getbetter'],
    description: 'Legt eine Aufgabe an, auf Wunsch mit Frist und Wichtigkeit (0 normal bis 3 sehr wichtig).',
    parameters: object(
      { title: TITLE, date: DATE, time: TIME, priority: { type: 'integer', minimum: 0, maximum: 3 }, note: NOTE },
      ['title'],
    ),
  },
  {
  ...
  },
  {
    ...
    parameters: object({ name: TITLE, per_week: { type: 'integer', minimum: 1, maximum: 7 } }, ['name']),
  },
  {
    name: 'add_shopping',
    apps: CALENDAR_APPS,
    description: 'Setzt Dinge auf die Einkaufsliste, je Ding ein Eintrag, Menge vorne (z. B. "2 Bananen").',
    parameters: object(
      {
        items: {
          type: 'array',
  ...
  },
  {
    ...
    parameters: object(
      { kind: { type: 'string', minLength: 1, maxLength: 60 }, minutes: { type: 'integer', minimum: 1, maximum: 600 } },
      ['kind', 'minutes'],
    ),
  },
  {
  ...
  },
  {
    ...
    parameters: object({ title: TITLE, amount: AMOUNT, due_date: DATE }, ['title', 'amount', 'due_date']),
  },
  {
    name: 'open_function',
    apps: EVERYWHERE,
...
function slim(schema) {
  if (schema.type === 'object') {
    return {
      type: 'object',
      properties: Object.fromEntries(Object.entries(schema.properties).map(([key, field]) => [key, slim(field)])),
...
function toolsFor(app) {
  return toolsOf(app).map((tool) => ({
    type: 'function',
    function: { name: tool.name, description: tool.description, parameters: slim(parametersFor(tool, app)) },
  }));
}
