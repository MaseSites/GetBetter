...
test('fragt den Vornamen nur, wenn er fehlt', () => {
  assert.deepEqual(setupSteps(false, false), ['name', 'assistant', 'style', 'ready']);
  assert.deepEqual(setupSteps(true, false), ['assistant', 'style', 'ready']);
});

test('fragt nach der Stimme nur, wenn es welche zu waehlen gibt', () => {
  assert.deepEqual(setupSteps(false, true), ['voice', 'name', 'assistant', 'style', 'ready']);
  assert.deepEqual(setupSteps(true, true), ['voice', 'assistant', 'style', 'ready']);
});
